            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-black text-green-600 uppercase tracking-[0.2em]">Latest Round Data</p>
                    </div>
                </div>
      ...
      <div className="bg-[#f95959] rounded-[2.5rem] p-6 text-white flex justify-between items-center shadow-xl relative overflow-hidden w-full">
        ...
        <div className="text-right z-10">
            <p className={cn("text-[10px] font-black uppercase tracking-[0.2em] opacity-90 mb-3")}>
                {timeLeft <= 5 ? "Processing..." : "Next Draw In"}
            </p>
            <div className="flex items-center gap-1.5 justify-end">
                {timeLeft <= 5 ? (
                    <div className="h-11 flex items-center justify-center px-4 rounded-xl bg-white text-red-600 font-black text-xl shadow-xl animate-pulse uppercase">
                        Last Result
                    </div>
                ) : (
            ...
            <TabsContent value="my" className="m-0 p-6 space-y-4 bg-secondary/5 min-h-[400px]">
                {myBets && myBets.length > 0 ? (
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-dashed border-border">
                                    <div className="flex items-center gap-4">
                                        <div className="space-y-0.5">
                                            <p className="text-[8px] font-bold text-muted-foreground uppercase">Selection</p>
                                            <p className="text-xs font-black uppercase italic text-foreground">{bet.selection}</p>
                                        </div>
                                        <div className="space-y-0.5">
                                            <p className="text-[8px] font-bold text-muted-foreground uppercase">Amount</p>
                                            <p className="text-xs font-black text-foreground">₹{bet.amount}</p>
                                        </div>
                                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 opacity-40">
                        ...
                        <p className="text-xs font-black uppercase tracking-[0.2em]">No bets placed yet</p>
                    </div>
                )}
            </TabsContent>
        </Tabs>
      ...
      <Dialog open={isBetPanelOpen} onOpenChange={setIsBetPanelOpen}>
        <DialogContent className="max-w-[420px] bg-background border-border rounded-[3.5rem] p-0 overflow-hidden z-[1000] shadow-2xl">
           <DialogHeader className="p-10 pb-0">
              <div className="flex items-center justify-between">
                <DialogTitle className="text-3xl font-black italic uppercase text-foreground tracking-tighter">Predict Selection</DialogTitle>
           ...
           <div className="p-10 space-y-8">
              <div className="bg-secondary p-8 rounded-[2.5rem] border border-border/50 flex items-center justify-between">
                  <div className="space-y-2">
                      <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Selected</p>
              ...
              <div className="space-y-4">
                  ...
                  <div className="relative">
                      ...
                      <Input type="number" placeholder="0.00" value={betAmount} onChange={(e) => setBetAmount(e.target.value)} className="h-16 bg-secondary border-none rounded-[1.5rem] pl-10 pr-6 text-xl font-bold" />
                  </div>
              </div>
              ...
              <Button onClick={handlePlaceBet} disabled={isBetting} className="w-full h-20 bg-primary hover:bg-primary/90 text-white font-black uppercase rounded-[2.5rem] shadow-xl flex items-center justify-center gap-3 active:scale-95 transition-all text-xl">
                  {isBetting ? <Loader2 className="animate-spin" /> : <><CheckCircle2 size={24} /> Confirm Bet</>}
              </Button>
           </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className={cn("max-w-[340px] p-0 border-none rounded-[2.5rem] overflow-hidden shadow-[0_30px_70px_rgba(0,0,0,0.5)] z-[2000] animate-in zoom-in duration-300", popup.isWin ? "bg-gradient-to-b from-[#2e7d32] to-[#1b5e20]" : "bg-gradient-to-b from-[#1565c0] to-[#0d47a1]")}>
            <div className="relative p-8 flex flex-col items-center text-center text-white space-y-6">
                ...
                <div className="space-y-1">
                    <h2 className="text-4xl font-black italic uppercase tracking-tighter">{popup.isWin ? "Winner!" : "Better Luck"}</h2>
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-70">{popup.isWin ? "Bonus Credited" : "Try Again Next Round"}</p>
                </div>

                <div className="bg-white/10 backdrop-blur-md rounded-3xl p-5 w-full border border-white/10 space-y-4">
                    <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Round Result</p>
                ...
                <div className="space-y-2">
                    {popup.isWin ? (
                        <><p className="text-[10px] font-black uppercase tracking-widest text-yellow-400">Winning Amount</p><div className="flex items-center justify-center gap-2"><Coins className="text-yellow-400" /><h3 className="text-5xl font-black italic tracking-tighter">₹{popup.amount}</h3></div></>
                ...
                <div className="w-full pt-4">
                    ...
                    <p className="text-[8px] font-black uppercase tracking-widest mt-3 opacity-40">Auto Close in {popupTimer}s</p>
                </div>
            </div>
        </DialogContent>
      </Dialog>
